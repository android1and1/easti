...
(function() {
  ...
  var Account, Daka, Nohm, Session, Store, accountModel, app, credential, crypto, dakaModel, express, filter, fs, hashise, http, path, pgrep, qr_image, redis, server, spawn, static_root, superpass;
  ...
  app.post('/user/login', async function(req, res) {
    var dbpassword, item, items, name, namebool, password, passwordbool, ref, timestamp;
    // reference line#163
    ...
    item = items[0];
    if (!item) { // currently this name isnt exists
      return res.json('没有这个用户.');
    }
    dbpassword = item.property('password');
    timestamp = new Date;
    if (hashise(password === dbpassword)) {
